...
Before calculating anything, we must check if there is any missing values in dataset. The function anyNA do this job efficeintly. 

```{r}
print(sapply(dataset, anyNA))
```
It can be seen that the columns 'repwt' and 'repht' have missing values. We can replace the missing values by average of the rest of the values in the column or we can just remove the rows containg the missing values. 
I think for this dataset which consist of reported weigth and reported height, it will be good if the missing values are replaced by average of the other values in column. 
...
```
Now we can have summary for males and females separetely
